...
def main():
	...
	for frame  in camera.capture_continuous(rawCapture,format='yuv',use_video_port=True):
	    ...
	    blurred = cv2.GaussianBlur(cropped, (7,3), 0)
            edges = cv2.Canny(blurred,1,35,apertureSize=3,L2gradient=False)
	    #edges[:,x:x+w] = 0
	    ...
	    if lines is not None:
            ...
            #cv2.imshow("Result", np.concatenate((cropped,edges,imclose,result[:,:,0]),0))
            cv2.namedWindow("Result2", cv2.WINDOW_AUTOSIZE)
