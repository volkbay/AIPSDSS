...
def main():
	...
	for frame  in camera.capture_continuous(rawCapture,format='yuv',use_video_port=True):
	    ...
	    #cropped = clahe.apply(cropped)
	    filtered = cv2.Sobel(blurred,cv2.CV_8U,2,0,ksize=3)
	    abs_sobel = np.absolute(filtered)
	    filtered = np.uint8(abs_sobel)
	    filtered2 = cv2.GaussianBlur(filtered, (5,5), 0)    
            edges = cv2.Canny(filtered2,20,50,apertureSize=3,L2gradient=False)
	    #edges[:,x:x+w] = 0
	    ...
	    imclose = cv2.morphologyEx(imopen, cv2.MORPH_CLOSE, kernel2)
            ...
            lines = cv2.HoughLines(imclose,1,np.pi/180,h/4)
	    ...
	    if lines is not None:
			...
			if theta*180/np.pi<20 or theta*180/np.pi>160:
                            ...
                            x2 = int(x0+(y2-y0)/a*(-b))
                            cur = x2 + x1
                            mid2 = [abs(x-cur) for x in mid]
                            min_val = min(mid2)
                            if (not mid) or ((min_val < -10) or (min_val>10)):
                                cv2.rectangle(result,(x1,y1),(x2,y2),(0,0,255),1)
				GPIO.output(led_pin, GPIO.HIGH)
				print("Found with rho-theta",rho,theta*180/np.pi)
				mid.append(cur)
            else:
                GPIO.output(led_pin, GPIO.LOW)
            
            #cv2.namedWindow("Result", cv2.WINDOW_AUTOSIZE)
            #cv2.imshow("Result", np.concatenate((cropped,filtered,edges,result[:,:,0]),0))
            cv2.namedWindow("Result2", cv2.WINDOW_AUTOSIZE)
            ...
            if cv2.waitKey(1) == 27:
                status = cv2.imwrite('/home/pi/Proje/image.png',result)
		print(status)
		break
	    t3 = time.time()
